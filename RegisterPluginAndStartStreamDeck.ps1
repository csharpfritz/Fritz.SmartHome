...
$pluginProjectFile = "$basePath\Fritz.SmartHome.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"Fritz.SmartHome`" project before calling this script.";
  exit 1;
}
